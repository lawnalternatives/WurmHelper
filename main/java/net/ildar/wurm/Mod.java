...
public class Mod implements WurmClientMod, Initable, Configurable, PreInitable {
    ...
    private final Map<ConsoleCommand, ConsoleCommandHandler> consoleCommandHandlers;
    ...
    private void handleActionCommand(String[] input) {
        ...
        if (action == null) {
            Utils.consolePrint("Unknown action abbreviation - " + abbreviation);
            showActionList();
            return;
        }
    ...
    private void printAvailableConsoleCommands() {
        ...
        commands.deleteCharAt(commands.length() - 1);
        Utils.consolePrint("Available custom commands - " + commands);
    }
